...
pub use kernel::{
    ...
    route_consistent_with_profile, select_frontier, trace_span_from_causal_protocol_event,
    trace_span_kind_from_causal_protocol_event_kind, truth_rewrite_of, validate_host_context,
...
pub use ports::{
    CausalProtocolHistoryPort, ConstraintProviderPort, ExecutorPort, HostDispatchPort,
    HostEffectHandler, PlannerPort, ProjectionPort,
};
pub use protocol::{
    ...
    HostRuntimeProfile, HostTaskSpec, HostTaskSpecBuilder, ImpactHardness, ImpactSetHash, Lane,
    LaneAdmission, LaneCapacity, LaneId, LaneRejection, Lease, LeaseId, LeaseRef, LeaseTable,
    LeaseTableError, LifecycleClass, LifecycleStage, LifecycleViolation, NotParallelReason,
    ObligationSet, Op, OpId, PairConflict, PartitionKey, PartitionRoute, PlanHash, PlanHashError,
    PredicateId, ProjectionReadRequest, RedactionClass, RedactionClassPolicy, RedactionPolicy,
    RedactionSurface, RedactionView, ResourceClaim, ResourceId, RouteId, RuntimeUpdate,
    RuntimeUpdateKind, Scope, SurfaceRedactionProfile, Tier, Timestamp, TraceAttribute, TraceSpan,
    TraceSpanId, TraceSpanKind, TruthAnchor, WhyNotParallel, WhyNotParallelInputs,
    WitnessAttestation, WitnessBinding, WitnessKind, WitnessRef, ALL_CAUSAL_PROTOCOL_EVENT_KINDS,
    CAUSLANE_HOST_API_VERSION, MAY_PROJECT_STAGE,
};
