...
`just formal-verify-all` is the default cross-target implementation gate. It
...
never patched to `pass` — P0-FM-002). Use `tools/formal-verify-all --profile
...
`formal/obligations/lifecycle_product_obligations.yaml` and
`docs/formal/formal_model_catalog.yaml` feed `tools/formal-discipline-check`,
which is run by `tools/formal-verify-all` after fresh coverage and
...
just formal-verify-all                            # default gate; runs every lane incl. Verus (no-cheating) + Lean4 (Lake/lean), always-on blocking
just formal-verify-lane fast_ci --dry-run         # provider-neutral lane entrypoint; lanes/unwinds come from formal/kani/profile.json
```
...
smoke. It writes live receipts under `formal/receipts/` (git-ignored), including
the core Alloy receipt and codegen receipt; see
`formal/receipts/examples/alloy-core.sample.json` for the schema.
...
in every `formal-verify-all` run (the base/rust/ci non-blocking exception was
dropped 2026-06-21).
...
```bash
causlane formal generate lean4   --bundle target/causlane/release_promote.bundle.json   --scenario contracts/scenarios/release_promote_success.scenario.yaml   --out formal/lean4/generated/release_promote_success.lean   --receipt formal/receipts/release_promote_success.lean4.codegen.json
...
causlane formal generate alloy --bundle target/causlane/release_promote.bundle.json --scenario contracts/scenarios/release_promote_success.scenario.yaml --out formal/alloy/generated/release_promote.als --receipt formal/receipts/release_promote.codegen.json
causlane formal stale-check --bundle target/causlane/release_promote.bundle.json --scenario contracts/scenarios/release_promote_success.scenario.yaml --generated formal/alloy/generated/release_promote.als --receipt formal/receipts/release_promote.codegen.json
```
