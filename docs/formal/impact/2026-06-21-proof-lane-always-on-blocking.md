...
- PR/issue: proof-lane blocking before exception expiry (branch `formal/proof-lane-blocking`)
- Owner: formal-core
- Date: 2026-06-21
...
```text
tools/formal-verify-all                                   (RUN_PROOF=1 always; drop skip branches)
docs/formal-exceptions.json                               (exceptions -> [])
docs/formal-exceptions.md
formal/obligations/lifecycle_product_obligations.yaml     (verus/lean4 proof_profile_required -> required)
...
docs/formal-readiness-status.md, formal/README.md, formal/verus/README.md,
formal/lean/README.md, docs/formal/03-lean4-verus-proof-obligations.md,
...
1. **Gate (`tools/formal-verify-all`):** `RUN_PROOF=1` unconditionally and the
...
- **Baseline confirmed by running, not assuming:** `tools/formal-verify-all`
