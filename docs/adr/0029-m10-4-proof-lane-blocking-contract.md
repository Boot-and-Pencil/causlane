...
Verus and Lean4 already run as blocking proof lanes in `scripts/check-verification-full.sh`,
...
Introduce `verification/formal-full/proof-lanes.json` as the machine-readable source for proof
lanes that must always run in `check-verification-full`.
