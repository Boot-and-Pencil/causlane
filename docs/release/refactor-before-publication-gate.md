...
```text
publish causlane-cli only after green CI and maintainer confirmation;
do not tag v0.0.1 or move to PUB6 until causlane-cli is indexed;
...
recorded in `docs/release/pub5-causlane-codegen-dry-run.md`. The publication
state is now `DryRunPassed(causlane-codegen)`. The next irreversible action is
publishing `causlane-codegen` after CI and maintainer confirmation.

Update 2026-06-28: `causlane-codegen 0.0.1` was published and indexed on
crates.io. Evidence is recorded in
`docs/release/pub5-causlane-codegen-publication.md`. The publication state is
now `Indexed(causlane-codegen)`. The next runbook crate is `causlane`.

Update 2026-06-28: the staged `causlane` dry-run passed. Evidence is recorded
in `docs/release/pub5-causlane-dry-run.md`. The publication state is now
`DryRunPassed(causlane)`. The next irreversible action is publishing
`causlane` after CI and maintainer confirmation.

Update 2026-06-28: `causlane 0.0.1` was published and indexed on crates.io.
Evidence is recorded in `docs/release/pub5-causlane-publication.md`. The
publication state is now `Indexed(causlane)`. The next runbook crate is
`causlane-cli`.

Update 2026-06-28: the staged `causlane-cli` dry-run passed. Evidence is
recorded in `docs/release/pub5-causlane-cli-dry-run.md`. The publication state
is now `DryRunPassed(causlane-cli)`. The next irreversible action is publishing
`causlane-cli` after CI and maintainer confirmation.
