...
`just verification-full` runs `verus --no-cheating` over the generated Verus
...
```
verification/formal-full/verus/core/kernel_state.rs       KernelState abstract state
verification/formal-full/verus/core/transitions.rs        transition relation
verification/formal-full/verus/core/invariants.rs         valid_state predicate (I-001..I-010)
verification/formal-full/verus/core/preservation.rs       transition_preserves_validity
verification/formal-full/verus/core/replay_soundness.rs   replay_accepts_implies_valid_protocol_trace
```

`verification/formal-full/verus/generated/release_promote_success.rs` is generated from Formal IR
and is verified on every `check-verification-full` run (a required, blocking lane).
...
doctor-full`; `scripts/check-verification-full.sh` then verifies Verus on every run.
